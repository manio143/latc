...
instance Print (MIdent a) where
  prt i e = case e of
    MIdent _ id -> prPrec i 0 (concatD [prt 0 id])
...
instance Print (TopDef a) where
  prt i e = case e of
    FnDef _ type_ mident args block -> prPrec i 0 (concatD [prt 0 type_, prt 0 mident, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
    ClDef _ mident classext clmembers -> prPrec i 0 (concatD [doc (showString "class"), prt 0 mident, prt 0 classext, doc (showString "{"), prt 0 clmembers, doc (showString "}")])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print (ClassExt a) where
  prt i e = case e of
    EmptyExt _ -> prPrec i 0 (concatD [])
    Ext _ mident -> prPrec i 0 (concatD [doc (showString "extends"), prt 0 mident])

instance Print (ClMember a) where
  prt i e = case e of
    Field _ type_ mident -> prPrec i 0 (concatD [prt 0 type_, prt 0 mident, doc (showString ";")])
    Method _ type_ mident args block -> prPrec i 0 (concatD [prt 0 type_, prt 0 mident, doc (showString "("), prt 0 args, doc (showString ")"), prt 0 block])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print (Arg a) where
  prt i e = case e of
    Arg _ type_ mident -> prPrec i 0 (concatD [prt 0 type_, prt 0 mident])
  prtList _ [] = (concatD [])
  prtList _ [x] = (concatD [prt 0 x])
...
instance Print (Item a) where
  prt i e = case e of
    NoInit _ mident -> prPrec i 0 (concatD [prt 0 mident])
    Init _ mident expr -> prPrec i 0 (concatD [prt 0 mident, doc (showString "="), prt 0 expr])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
...
instance Print (Expr a) where
  prt i e = case e of
    ECast _ type_ expr -> prPrec i 7 (concatD [doc (showString "("), prt 0 type_, doc (showString ")"), prt 0 expr])
    ...
    EApp _ expr exprs -> prPrec i 6 (concatD [prt 0 expr, doc (showString "("), prt 0 exprs, doc (showString ")")])
    EMember _ expr mident -> prPrec i 6 (concatD [prt 0 expr, doc (showString "."), prt 0 mident])
    ENew _ type_ -> prPrec i 6 (concatD [doc (showString "new"), prt 0 type_])
    EArr _ expr1 expr2 -> prPrec i 6 (concatD [prt 0 expr1, doc (showString "["), prt 0 expr2, doc (showString "]")])
